# ESQUADЯO — histórico das decisões

O porquê de cada coisa do painel: o que foi tentado e falhou, as medidas
tiradas do desenho, e os erros que não devem se repetir.

Foi separado do [`ESTADO.md`](ESTADO.md) em 27/08/2026. O ESTADO ficou com o que
se consulta trabalhando — armadilhas do UXP, regras ativas, itens abertos. Este
arquivo guarda a narrativa, que é longa e não muda mais.

**Nada aqui é instrução para hoje.** Se este arquivo e o ESTADO discordarem, o
ESTADO manda — e o código manda nos dois.

---

## Por que a lista é assim

As regras em vigor estão em `ESTADO.md`. Aqui está o motivo de cada uma.
...
## Por que o nome do arquivo é assim
...
**Manter esse formato de comentário importa:** é por ele que a ferramenta de
calibração monta os controles, e um token sem ele fica sem slider.

Essa ferramenta (`gerar-playground.py`) saiu do versionamento em 26/08: calibrar
espaçamento e tipografia é atividade de design, não do build. Ela roda o painel
de verdade — JS real, embrulhado num `require` de mentira — e é a única coisa
que avisa de raio que ovalizaria no UXP, porque essa regra depende da altura
renderizada e não dá para conferir no CSS.
...
**O ícone é sempre o mesmo desenho: o que muda é quanto ele aparece.** Quem não
está escolhido recua para 20%, quem está fica cheio e ganha a companheira. O
hover só sobe para 55%, sem caixa e sem cor nova, para não competir com o "está
selecionado".

**E o recuo é feito na COR, não em `opacity`** — `rgba()` no `color`, e o
desenho do ícone segue por `currentColor`. Este parágrafo dizia o contrário até
26/08 e era texto velho, sobrevivente de antes da correção: ele contradizia a
tabela de armadilhas três seções acima, que diz que o UXP ignora `opacity`. Quem
manda é o `styles.css`.
...
Saiu deste repositório em 26/08/2026, para o repositório privado
[`esquadro-apresentacao`](https://github.com/gabi-lapisraro/esquadro-apresentacao)
(mesma conta, mesma licença — cópia da LAPISRARO). Não é engenharia do
plugin: são o `apresentacao.html` publicado como artifact, e os geradores de
PDF e PPTX. O conhecimento sobre eles — o bug do Menlo no PDF headless, o
`print-color-adjust`, o `Milo` vs. `Milo OT` — foi junto, em `docs/NOTAS.md`
daquele repositório, para não se perder.

---

## Erros de análise, registrados para não repetir
